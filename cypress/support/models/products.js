Cypress.Commands.add('verifyProduct', (line, product) => {
    cy.get('[class="css-4t9hm0"]').eq(line).within(() => {
...
Cypress.Commands.add('verifyProductsFix', (products) => {
    Cypress._.forEach(products, (product, index) => {
        cy.get('[class="css-4t9hm0"]').eq(index).within(() => {
            cy.get('[data-cy="name"]').should('have.text', product.name);
            cy.get('[data-cy="price"]').should('have.text', product.price);
        });
